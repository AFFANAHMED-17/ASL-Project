...
model = models.Sequential([
    ...
    layers.Dense(len(asl_label_map), activation='softmax')  # Output layer for number of classes
])
...
y_pred = model.predict(X_test_asl)
y_pred_classes = np.argmax(y_pred, axis=1)

# Print classification report
print(classification_report(y_test_asl, y_pred_classes, target_names=list(asl_label_map.values())))

# Print confusion matrix
print("Confusion Matrix:")
print(confusion_matrix(y_test_asl, y_pred_classes))
